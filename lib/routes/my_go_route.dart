...
class MyGoRouter extends GoRouter {
  final _didPopNotifier =
      RouteChangeNotifier<Route<dynamic>, Route<dynamic>?>();
  final _didPushNotifier =
      RouteChangeNotifier<Route<dynamic>, Route<dynamic>?>();
  final _didRemoveNotifier =
      RouteChangeNotifier<Route<dynamic>, Route<dynamic>?>();

  MyGoRouter({required List<GoRoute> routes, String initialLocation: '/'})
      : super(routes: routes, initialLocation: initialLocation);

  addDidPopListener(RouteChange<Route<dynamic>, Route<dynamic>?> listener) {
    _didPopNotifier.addListener(listener);
  }

  removeDidPopListener(RouteChange<Route<dynamic>, Route<dynamic>?> listener) {
    _didPopNotifier.removeListener(listener);
  }

  addDidPushListener(RouteChange<Route<dynamic>, Route<dynamic>?> listener) {
    _didPushNotifier.addListener(listener);
  }

  removeDidPushListener(RouteChange<Route<dynamic>, Route<dynamic>?> listener) {
    _didPushNotifier.removeListener(listener);
  }

  addDidRemoveListener(RouteChange<Route<dynamic>, Route<dynamic>?> listener) {
    _didRemoveNotifier.addListener(listener);
  }

  removeDidRemoveListener(
      RouteChange<Route<dynamic>, Route<dynamic>?> listener) {
    _didRemoveNotifier.removeListener(listener);
  }

  @override
  void didPop(Route<dynamic> route, Route<dynamic>? previousRoute) {
    _didPopNotifier.notifyListeners(route, previousRoute);
    super.didPop(route, previousRoute);
  }

  @override
  void didPush(Route<dynamic> route, Route<dynamic>? previousRoute) {
    _didPushNotifier.notifyListeners(route, previousRoute);
...
class RouteChangeNotifier<T, P> {
  int _count = 0;
  List<RouteChange<T, P>?> _listeners = List.filled(0, null);
  ...
  /// [RouteChangeNotifier] not being able to determine which listener is being
  ...
  void addListener(RouteChange<T, P> listener) {
    assert(_debugAssertNotDisposed());
    if (_count == _listeners.length) {
      if (_count == 0) {
        _listeners = List<RouteChange<T, P>?>.filled(1, null);
      } else {
        final List<RouteChange<T, P>?> newListeners =
            List<RouteChange<T, P>?>.filled(_listeners.length * 2, null);
  ...
  void _removeAt(int index) {
    ...
    if (_count * 2 <= _listeners.length) {
      final List<RouteChange<T, P>?> newListeners =
          List<RouteChange<T, P>?>.filled(_count, null);
  ...
  void removeListener(RouteChange<T, P> listener) {
    assert(_debugAssertNotDisposed());
    for (int i = 0; i < _count; i++) {
      final RouteChange<T, P>? _listener = _listeners[i];
  ...
  void notifyListeners(T route, P previousRoute) {
    assert(_debugAssertNotDisposed());
    if (_count == 0) return;
    ...
    for (int i = 0; i < end; i++) {
      try {
        _listeners[i]?.call(route, previousRoute);
        ...
        FlutterError.reportError(FlutterErrorDetails(
          ...
          informationCollector: () => <DiagnosticsNode>[
            DiagnosticsProperty<RouteChangeNotifier>(
    ...
    if (_notificationCallStackDepth == 0 && _reentrantlyRemovedListeners > 0) {
      ...
      if (newLength * 2 <= _listeners.length) {
        ...
        final List<RouteChange<T, P>?> newListeners =
            List<RouteChange<T, P>?>.filled(newLength, null);

        int newIndex = 0;
        for (int i = 0; i < _count; i++) {
          final RouteChange<T, P>? listener = _listeners[i];
          if (listener != null) {
