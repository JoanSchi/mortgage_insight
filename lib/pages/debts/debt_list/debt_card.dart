...
class SchuldCard<T> extends StatelessWidget {
  final T item;
  ...
  Widget build(BuildContext context) {
    switch (item.runtimeType) {
      case AflopendKrediet:
        {
          return AflopendKredietCard(ak: item as AflopendKrediet);
        }
      case DoorlopendKrediet:
        {
          return DoorlopendKredietCard(dk: item as DoorlopendKrediet);
        }
      case VerzendhuisKrediet:
        {
          return VerzendKredietCard(vk: item as VerzendhuisKrediet);
        }

      case OperationalLeaseAuto:
        {
          return LeaseAutoCard(ola: item as OperationalLeaseAuto);
        }
    }

    return MoCard(
        top: Text('Unknown runtimeType'),
        body: Text(
          ':{',
          textScaleFactor: 1.0,
        ));
...
class _LeaseAutoCardState extends ConsumerState<LeaseAutoCard> {
  ...
  @override
  Widget build(BuildContext context) {
    final ola = widget.ola;
    final theme = Theme.of(context);

    final huidigeDatum = DateUtils.dateOnly(DateTime.now());
    final dateStatus = ola.dateStatus(huidigeDatum);

    final overzicht = ola.overzicht(huidigeDatum);

    List<Widget> overzichtList;
    if (overzicht['layout'] == 1) {
      overzichtList = [
        Text(
    ...
    final beginDatum =
        DateFormat.yMMMMd(localeToString(context)).format(ola.beginDatum);

    final eindDatum =
        DateFormat.yMMMMd(localeToString(context)).format(ola.eindDatum);
          ...
          Positioned(
            ...
            child: Column(
              ...
              children: [
                ...
                if (ola.omschrijving.isNotEmpty)
                  Text(
                    '${ola.omschrijving}',
                  )
              ],
              ...
              child: ConstrainedBox(
                  ...
                  child: Text(
                      nf.parseDoubleToText(
                          ola.maandLast(DateTime.now()), '#0.0'),
                      textAlign: TextAlign.center,
                      style: theme.textTheme.bodyText2
                          ?.copyWith(color: _maandLastTextColor(dateStatus)))),
            ),
  ...
  void onSelected(String selected) {
    switch (selected) {
      case 'edit':
        ...
        break;
      case 'delete':
        ref.read(hypotheekContainerProvider.notifier).removeSchuld(widget.ola);
        break;
    }
  }
}
