...
class ToevoegenKostenButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final border = (roundedLeft != 0.0 || roundedRight != 0.0)
...
class _DefaultKostenRowItemState extends State<DefaultKostenRowItem> {
  ...
  late TextEditingController _tecName =
      TextEditingController(text: waarde.name);

  late FocusNode _fnName = FocusNode()
    ..addListener(() {
      if (!_fnName.hasFocus) {
  ...
  late TextEditingController _tecWaarde = TextEditingController(
      text: waarde.value == 0.0 ? '' : nf.parseDblToText(waarde.value));

  late FocusNode _fnWaarde = FocusNode()
    ..addListener(() {
        ...
        children: [
          Expanded(
            child: TextFormField(
              controller: _tecName,
              focusNode: _fnName,
              decoration: InputDecoration(hintText: '...', labelText: 'Naam'),
      ...
      itemBuilder: (BuildContext context) {
        ...
        return [
          ...
          PopupMenuItem<SelectedMenuPopupIdentifierValue<String, String>>(
              ...
              child: KostenMenuItem(
                icon: const Icon(Icons.delete, color: Color(0XFF154273)),
                text: 'Verwijderen',
              )),
        ];
      },
    ...
    final children = l
        .map((_SelectedItem<Waarde> e) => SizedBox(
              ...
              child: CheckboxListTile(
                value: e.selected,
                title:
                    Text('${e.value.name.isEmpty ? 'Overige' : e.value.name}'),
